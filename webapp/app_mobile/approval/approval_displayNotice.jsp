...
String range = (String) request.getParameter("readRange");
...
function getDisplayList() {
	if(moreFlag != false){
 	 ...
 	 $.ajax({
			...
			url:'<%=webUri%>/app/list/webService/approval/ListDisplayNotice.do',
...
function setDisplayList(data){
	
	if(pageSetting == 1){
		$("#approvalDisplayList").html("");
	}
	
 	if(data.ListVos.length == 0){
		addHtml += "<li class='group'><a href=''>";
		addHtml += "<div class='rf'><p class='title'>공람게시 목록이 없습니다.</p></div></a>";		
	}else{
				
		for(var i = 0; i < data.ListVos.length; i++){
			addHtml += "<li class='group'>";
			addHtml += "<a>";
			addHtml += "<img src='<%=iam_url%>/acube/iam/identity/userinfoimage/Select.go?requestImage=picture&userId="+data.ListVos[i].publisherid+"' alt='이미지' class='circleImg'/>";
			...
			addHtml += "<p class='status'><span>"+data.ListVos[i].publishername+"</span><span>|</span><span>"+data.ListVos[i].publishdate+"</span></p>" ;
			addHtml +=	"</div></a></li>";		
		} 
	}
 	
 	processCount += data.ListVos.length;
	$("#approvalDisplayList").html(addHtml);
...
function viewAppDoc(docid){
	...
	document.location.href = '<%=webUri%>/app/list/webservice/approval/selectAppDoc.do?docId='+docid+'&D1='+d1+'&lobCode=LOB012';				
...
function goApprovalBox(index){
	...
	case 6:                       
		...
		break;                    
	case 7:                       
		document.location.href = '<%=webUri%>/app_mobile/approval/approval_displayNotice.jsp?readRange=DRS005&D1='+d1;				//공람게시 (회사)
		break;
	}
}
function searchArea(){
	...
	<div id="wrapper" style="overflow:hidden;">
		<div id="scroller">
			...
			<div class="sub_content app">
				<ul class="list V_Limg_list Rdiv group" id="approvalDisplayList">
					<!-- <li class="group">
						<a href="#a">
			...
			<div class="moreNtop">
				<a href="#none" class="more_bottom" onclick="getDisplayList();">더보기 <span id="listCount"></span><span>/</span><span id="listTotalCount"></span></a>
