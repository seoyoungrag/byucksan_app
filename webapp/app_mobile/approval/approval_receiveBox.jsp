...
/* 	$("#top_btn").click(function(){
		alert("click");
		$("#titleBar").animate({scrollTop:0},'slow');
	...
	if(moreFlag != false){
		pageSetting++;
	 	 $.ajax({
				...
				url:'<%=webUri%>/app/list/webservice/approval/ListApprovalWaitBox.do',
...
function setReceiveList(data){
	
	if(pageSetting == 1){
		$("#approvalReceiveList").html("");
	}
	
 	if(data.ListVos.length == 0){
		addHtml += "<li class='group'><a href=''>";
		addHtml += "<div class='rf'><p class='title'>수신문서 목록이 없습니다.</p></div></a>";		
	}else{
	...
	$("#approvalReceiveList").html(addHtml);	
...
function viewAppDoc(docid){
	...
	if(docid != null){
		 if(docid.substring(0,3) == "APP"){
			//생산문서
			document.location.href = '<%=webUri%>/app/list/webservice/approval/selectAppDoc.do?docId='+docid+'&D1='+d1+'&lobCode=LOB003&isreceiveBox=Y';	
		}else{
			//접수문서
			document.location.href = '<%=webUri%>/app/list/webservice/approval/EnforceDocument.do?docId='+docid+'&D1='+d1+'&lobCode=LOB003&isreceiveBox=Y';
		}	
	}
...
<body>
	...
	<div class="sub_top msn_top" id="header">
	...
	<div id="wrapper" style="overflow:hidden;">
		<div id="scroller">
			...
			<div class="sub_content app">
				<ul class="list V_Limg_list Rdiv group"  id="approvalReceiveList">
					<!-- <li class="group">
						<a href="#a">
			...
			<div class="moreNtop">
				<a href="#none" id="moreNtopPageView" class="more_bottom" onclick="getReceiveList();">더보기 <span id="listCount"></span><span>/</span><span id="listTotalCount"></span></a>
