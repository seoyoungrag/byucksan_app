...
//		         $("#mobileYn", "#approvalitem").val("N"); 
	...
	function selectDocInfo(type) {
        ...
        if('Read' == type){
           //조회
		   docinfoWin = openWindow("docinfoWin", "<%=webUri%>/app/enforce/selectDocInfo.do?lobCode=<%=lobCode%>&docState=<%=docState%>&docId=" + $("#docId", "#approvalitem").val() + "&receiverOrder=" + $("#receiverOrder", "#approvalitem").val(), 550, height);

        }else{
           //저장
           docinfoWin = openWindow("docinfoWin", "<%=webUri%>/app/enforce/popupDocInfo.do?docState=<%=docState%>&bindId=<%=enfDocVO.getBindingId()%>", 550, height);
        }
    }
		...
		<div id="scroller">
			...
			<div class="sub_content">
				<ul class="list V_Limg_list Rdiv group">
					<li class="group">
						<%-- <a href="#none"><img src="http://211.168.82.16:7001/acube/iam/identity/userinfoimage/Select.go?requestImage=picture&userId=<%=drafterUid%>" alt="이미지"/>
							<div class="rf">
					...
					%>	
						<div class="file_attach">
							...
							<p class="file_attach_bg">
								<a class="input_attach"  href="#" onclick="fileDownload('<%=fileVO.getFileId()%>','<%=fileVO.getFileName()%>','<%=fileVO.getDisplayName()%>','<%=fileVO.getFileType()%>'); return false;">
