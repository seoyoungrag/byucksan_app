...
<%
/** 
 *  Class Name  : EnvPubInspectPop.jsp 
 *  Description : 공람자그룹 등록(팝업)
...
$(document).ready(function() { 	
	...
	if (opener != null && opener.getPubViewGroupName && opener.getMode() == "U") {
		$('#groupName').val(opener.getPubViewGroupName());
	}
...
function insertUser(results){
	...
	if(results.length > 0){
		for(var i = 0; i < results.length; i++){
			...
			row += "<td class='ltb_center l_td2' width='94'>"+userPosition+"</td>";
			row += "<td class='ltb_center l_td2'>"+user.userName+"</td></tr>";
			tbl.append(row);
		}
	} else{
...
function MakePubReader(idn, recvObj){
	...
	row += "<td width='287' class='ltb_center'>" +recvObj.pubReaderDeptName + "</td>"; 
...
function sendOk(){
	...
	if(lines.length === 0){
		alert('<spring:message code="approval.msg.applines.nopubviewers" />');
		return;
	}
	
	pubViewerline = makeRecvLine(lines.children());
	//alert(pubViewerline);
	if (opener != null && opener.setPubViewGroupInfo && opener.setPubViewGroupName && opener.getMode) {
		opener.setPubViewGroupInfo(pubViewerline);
		opener.setPubViewGroupName($('#groupName').val());

		if (opener.getMode() == "I" ) {
			opener.insertPubViewGroup();
		} else if (opener.getMode() == "U") {
			opener.updatePubViewGroup();
		}		
		window.close();
	}
}
...
function makeRecvLine(lines){
	...
	for(var i = 0; i < lines.length; i++){
		...
		//strRt += (line.getAttribute('pubReaderDeptId') 	+ String.fromCharCode(2) + line.getAttribute('pubReaderDeptName') 	+ String.fromCharCode(2) + line.getAttribute('pubReaderRole') 		+ String.fromCharCode(2) + String.fromCharCode(2) + line.getAttribute('pubReaderOrder') 		+ String.fromCharCode(2));
...
function setPubReader(){
	...
	if (opener != null && opener.getTxtPubViewLines) {
		var readersinfo = opener.getTxtPubViewLines();
...
</div>	<acube:outerFrame>

	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="pop_table05">
		<tr>
			<td>
			<span class="pop_title77"><spring:message code="approval.title.apppublic" /></span></td></tr>
	         <!-- 여백 시작 -->
			...
			<tr>
		    	<td height="3" colspan="2" background="<%=imagePath%>/dot_search1.gif"></td>
		    </tr>
			<tr>		
				<td width="51%">		
		...
		<div id="divInternal" style="height:200px;">
			...
			<tr>
				...
				<tr><td>
				<table class="table table_border" width="100%" border="0" cellpadding="0" cellspacing="0">
					<thead>
				...
				<table id="tbUsers" width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed;background-color:#cccccc;">
					<tbody />
				</table>
				</div>
				</td>
		...
		<!-- 여백 끝 -->
		<!-- 공람자 목록 -->
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="basic_box2">
			<tr>
				<td>
					<acube:tableFrame class="" width="100%" border="0" cellpadding="1" cellspacing="0" >
						<tr>
							...
							<div id="divPubView" style="height:120px; overflow-x:hidden; overflow-y:auto; background-color:#FFFFFF;">
								<table id="tbRecvLines" width="100%" border="0" cellpadding="0" cellspacing="1" style="table-layout:fixed;background-color:#cccccc;">
									<tbody />
								</table>
							</div>
							</td>
