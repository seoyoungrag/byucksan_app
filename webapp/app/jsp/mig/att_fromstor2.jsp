...
if (!bdemo)
{
	out.println("DEL : " + uiBean.encodeByLocale(request.getParameter("DEL")) + "<BR>"); 
	out.println("TYPE : " + uiBean.encodeByLocale(request.getParameter("TYPE")) + "<BR>"); 
	out.println("FILECNT : " + uiBean.encodeByLocale(request.getParameter("FILECNT")) + "<BR>");
	out.println("FOLDERID : " + uiBean.encodeByLocale(request.getParameter("FOLDERID")) + "<BR>"); 
	out.println("DOCID : " + uiBean.encodeByLocale(request.getParameter("DOCID")) + "<BR>"); 
}
...
{
	/* Test로 찍는 메시지로 판단
	out.println("<SCRIPT>alert('이름 : " + uiBean.encodeByLocale(request.getParameter("FILENAME")) + "');");
	out.println("alert('제목 : " + uiBean.encodeByLocale(request.getParameter("FILETTL")) + "');</SCRIPT>");*/
	out.println("FILEID : " + uiBean.encodeByLocale(request.getParameter("FILEID")) + "<BR>"); 
	out.println("FILENAME : " + uiBean.encodeByLocale(request.getParameter("FILENAME")) + "<BR>"); 
	out.println("FILETTL : " + uiBean.encodeByLocale(request.getParameter("FILETTL")) + "<BR>");
	//out.println("DOCTYPENM : " + varDocTypeNM + "<BR>");
}
...
System.out.println("Debug Data Presentation");
if ( varFleInfo != null )
{
   for (int qq=0; qq < varFleInfo.length; qq++)
   {
      ...
      {
         System.out.println("varFleInfo[" + qq + "][" + ww + "] = " + varFleInfo[qq][ww]);
      }
   }
}
...
if ( varFile != null )
{
   for (int qq=0; qq < varFile.length; qq++)
   {
      ...
      {
         System.out.println("varFile[" + qq + "][" + ww + "] = " + varFile[qq][ww]);
      }
   }
}
System.out.println("End of Debug Data Presentation");
...
out.println("uiBean.getUserID() : " + uiBean.getUserID());
out.println("SecID : " + iSecID);
System.out.println("uiBean.getUserID() : " + uiBean.getUserID());
System.out.println("SecID : " + iSecID);
...
if ( bDrm==true )   // DRM 문서인 경우
{
   try
   {
      out.println("********DRM applied");
      System.out.println("********DRM applied");
      varFleInfo = lq.getFilesDrmEx(sDocId, sDocId, varFleInfo, uiBean.getStrCurDb());

      if ( varFleInfo != null )
      {         
         System.out.println("varFleInfo length : " + varFleInfo.length);
         out.println("varFleInfo length : " + varFleInfo.length);
...
System.out.println("End of getFiles");
///////////////////////////////////////
%>

<!-- DRM Section ACUBE/Security 2002-07-05 add by junhyuk : MaryAny DRM-->
<%@ include file="./drm/SC_DRMDocEnc.jsp"%>
<!----------------------------------------------------------------------->
...
if ( sOpType.equalsIgnoreCase("F") && varFleInfo != null && sFileCnt.equalsIgnoreCase("1") )
{
	...
	if (sDefaultMode.equals("")&& sTmpCase.endsWith(".MGR") )	
	{
		...
		try
		{
         ...
         jsftpnative.JSFTPNative jSFTP = new jsftpnative.JSFTPNative();
			...
			if (nConnID < 0) { //연결실패
				System.out.println("SFTP Connection Fail in SEND To OM for MGR View");
			}
			else 
			{
				...
				{
					//getBaseDir받아오기 실패
					System.out.println("Fail to Get BaseDir.");
				} 
				else 
				{
					strBaseDir = jSFTP.JSFTP_getBaseDirValue();	
					System.out.println("BaseDir : " + strBaseDir);
		...
		{
			System.out.println("Error in GetBaseDir Or Upload MGR File -- See SFTP log");
		}		
		%>
	...
	else if  (sDefaultMode.equals("")&& sTmpCase.endsWith(".DL") )	
	{
		...
		{
			System.out.println("Error in GetBaseDir Or Upload Dl File -- See SFTP log");
		}		
		%>
			<SCRIPT language=javascript>
   ...
   // 2002-12-16 add by junhyuk : 결재 XML 문서일 경우 in mySingle V3
	else if ( sTmpCase.endsWith(".MGR.XML") || sTmpCase.equalsIgnoreCase("RECEIVEINFO.XML") || sTmpCase.equalsIgnoreCase("TRACEINFO.XML") )
	{
		...
		out.println("mySingle : MGR.XML, Mail.XML Mode<BR>");
      ...
      sXmlFilePath = dmViewUtil.getHostURI(request) + SERVLET + "/tmp/" + tGUID.toString() + ".xml" + "?filepath=" + sOut;
		...
		out.println("'Output XML Server Path : " + sOut);
		out.println("'Output XML Web Path : " + sXmlFilePath);
		...
		try
		{
         ...
         // 임시 추가
         System.out.println("XML Convert");
         try
         {
         ...
         {
            System.out.println("Not UTF-8 Document");
         }
         ///////////////////////////////////////////////
	...
	else 
	{
		...
		if ( sTmpCase.endsWith(".XML") && ( varDocTypeNM.equalsIgnoreCase("A") || varDocTypeNM.equalsIgnoreCase("E") || varDocTypeNM.equalsIgnoreCase("G") ) )
		{
			...
			{
				varTmpFleInfo = lq.getAttachFileInfo( varFleId[0], uiBean.getStrCurDb());
				out.println(varTmpFleInfo[0] + " " + varTmpFleInfo[1] + " " + varTmpFleInfo[2] + "<BR>"); 
			}
			catch ( Exception e )
			{
				lm.writeLog("att_fromstor.jsp - Fail in getAttachFileInfo : " + varFleId[0], "DmListQueryEJB");
...
else if ( sOpType.equalsIgnoreCase("PDFVIEW") && varFleInfo != null && sFileCnt.equalsIgnoreCase("1") )
{
   /*
	...
	varFile[i][3] = varFleName[i];
   */
   out.println("Input Value : " + varFile[0][2]);
   String sReturnPath = "";
   ...
   */
   ...
   out.println("PDF Result : " + sReturnPath);
   // Error Treat
   if ( sReturnPath.equals("") )
   {
      ...
      // PDF 생성도중 에러 발생 or PDF 변환 대상이 아님
		out.println("<SCRIPT language=javascript>");
		out.print("	alert(\"" + rsc.getProperty("att14") );
		out.println("\");");
		out.println("</SCRIPT>");
      lOpenMode = 0;    // Do not Use ActiveX Control
   }
...
<%!
	...
	{
		System.out.println("Get UUID Name From Original Name's Extension");
		String sName = null;
...
<%! 
	...
	{
		System.out.println("File unCompressed By GZIP : " + sPath);
		String sTarget = null;
		...
		System.out.println("Target to Uncompress : " + sTarget);
		...
		try
		{
			...
			System.out.println("error : FileNotFoundException" + e); 
			bCompBool = false;
		} catch(IOException e) { 
