...
<SCRIPT LANGUAGE="javascript">
	var origAdminInfo = '${row.adminInfo}';
	var adminInfo='${row.adminInfo}';
	var origAuthInfo = '${row.authInfo}';
	var authInfo='${row.authInfo}';
	var chkAll = 0;
	var isBindAdmin = 'Y';
	...
	function goUnitTreePage() {
		var params = 'deptId=' + "${row.compId}" + '&adminSelectDept=N';
		var url = '<%=webUri%>/app/bind/unit/simple/shareUnitListTree.do?'+params;
	...
	function edit() {
		...
		$.ajax({
			url : '<%=webUri%>/app/bind/updateShare.do',
			type : 'POST',
			dataType : 'json',
			data : {
				unitId : unitId,
				...
				adminInfo : adminInfo,
				origAdminInfo : origAdminInfo,
				authInfo : authInfo,
				origAuthInfo : origAuthInfo,
				bindOrder : bindOrder
			},
			success : function(data) {
				if(data.success) {		
					...
					window.close(); --%>
					fncBindUpdateCallback(bindId, bindName) 
				} else {
					if(data.msgId) {
						alert(data.msg);
					} else {
	...
	$(document).ready(function() { 
		...
		var description = replaceAll('${row.description}',"<br />","\r\n");
		...
		if(sendType == 'DST004'){
			document.all['docType'].disabled = true;
			document.all['retentionPeriod'].disabled = true;
			//분류체계설명
			$('#description').attr('readonly', true);
			$('#description').attr('class','input_read');
			//$('#description').attr('style', 'background-color:rgb(241, 241, 241)');
			
			//문서형태
			$('#docType').attr('class','input_read');
			
			//캐비넷명
			$('#bindName').attr('readonly', true);
			$('#bindName').attr('class','input_read');
			//$('#bindName').attr('style', 'background-color:rgb(241, 241, 241)');
			//순서
			$('#bindOrder').attr('readonly', true);
			$('#bindOrder').attr('class','input_read');
			//$('#bindOrder').attr('style', 'color:rgb(241, 241, 241)');
			//종료년도
			$('#expireYear').attr('class','input_read');
			//보존기간
			$('#retentionPeriod').attr('class','input_read');			
		}
	});
...
<acube:outerFrame popup="true">
	<form id="bindForm" name="bindForm" method="POST" action="<%=webUri%>/app/bind/update.do" onsubmit="return false;">
	<input type="hidden" name="bindId" value="${row.bindId}"/>
	<input type="hidden" name="deptId" value="${row.deptId}"/>
	<input type="hidden" name="adminInfo" id="adminInfo" value="${row.adminInfo}" />
	<input type="hidden" name="authInfo" id="authInfo" value="${row.authInfo}" />
				...
				<acube:tableFrame>
					<tr bgcolor="#ffffff">
						...
						<table width="100%" border="0" cellspacing="1" cellpadding="1">
							<tr bgcolor="#ffffff">
								<td><input style="width:100%" type="text" class="input" id="bindName"  name="bindName" value="${row.bindName}" size="49"></td>
							</tr>
						</table>
						</td>
					</tr>
					...
					<tr bgcolor="#ffffff"> 
						<td width="150" class="tb_tit" nowrap="nowrap"><spring:message code='bind.obj.unit.description'/></td>
						<td height=68><table width="100%" border="0" cellspacing="1" cellpadding="1">
								<tr bgcolor="#ffffff">
									<td><textarea id="description" name="description" style="width:100%; height:100%; overflow:auto" rows="5" onkeyup="checkInputMaxLength(this, '', 400)"></textarea></td>
								</tr>
							</table>
						</td>
					</tr> 
					...
					<tr bgcolor="#ffffff">
						<td width="150" class="tb_tit" nowrap="nowrap"><spring:message code='bind.obj.order'/></td>
						<td><table border="0" cellspacing="1" cellpadding="1">
								<tr bgcolor="#ffffff">
									<td><input type="number" class="input" id="bindOrder" name="bindOrder" size="20" value="${row.bindOrder}"></td>
								</tr>
							</table>
						</td>
					</tr>
						...
						<table border="0" cellspacing="1" cellpadding="1">
							<tr bgcolor="#ffffff">
								<td style="padding-left:-1px;">
									<input type="text" class="input_read" id="unitName" name="unitName" value="${row.unitName}" readonly size="35">
								</td>
								<input type="hidden" id="unitId" name="unitId" value="${row.unitId}"/>
								<input type="hidden" id="originalUnitType" value="${row.unitType}" />
								<input type="hidden" id="modifiedUnitType" />
								<c:if test="${selectSendType!= 'DST004'}">
			...
			<td>
				...
				<acube:buttonGroup>
					<acube:button onclick="javascript:edit();"
						value='<%= m.getMessage("bind.button.edit", null, langType) %>' type="2" class="gr" />
				</acube:buttonGroup>
				<!-------기능버튼 Table E --------->
			</td>
		</tr>
