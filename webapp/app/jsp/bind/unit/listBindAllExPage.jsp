...
<%-- <% if(roleCode.indexOf(systemAdminCode) > -1 || roleCode.indexOf(appSystemAdminCode) > -1) { %>	 --%>
	...
	function goSelectComp() {
		...
		form.action = '<%=webUri%>/app/bind/unit/ex/listAllBind.do?deptId=' + '${compId}' + '&adminSelectDept=N';
		form.target = '_self';
		form.submit();
	}
	...
	function setDeptInfo(obj) {
		if (typeof(obj) == "object") {
			if('ChangeDept' == openType) {
				...
				form.searchType.value = "";
				form.searchValue.value = "";
				form.action = '<%=webUri%>/app/bind/unit/ex/listAllBind.do?deptId=' + obj.orgId + '&adminSelectDept=Y';
				form.target = '_self';
				form.submit();
			} 
		}
...
<acube:outerFrame>
	<form id="listForm" name="listForm" method='post' action='<%=webUri%>/app/bind/unit/ex/listAllBind.do'>
						...
						<tr>
							...
							<%
							    try {
									    ...
									    acubeList.setColumnWidth("30,300,*");
									    acubeList.setColumnAlign("center,center,left");
									    ...
									    if (rows.size() == 0) {
											...
											for (int i = 0; i < rows.size(); i++) {
											    ...
											    row.setAttribute(rowIndex, "style", "text-align:center; line-height:28 px;"); //텍스트 위치 변경 jskim_20150526
											}
									    }
