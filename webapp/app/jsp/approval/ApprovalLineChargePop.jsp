...
function insertUser(results){
	...
	if(results.length > 0){
		for(var i = 0; i < results.length; i++){
			...
			//	직위 클릭시 사용자 정보 팝업창 호출		20150728_csh
...
function onListClick(obj){ 
		...
		addApprovalLine();	// 2015.07.27 _ 결재 지정자 더블 클릭에서 원 클릭 추가로 변경
...
function addApprovalLine(){
	...
	for(var i = 0; i < appColl.length; i++) {
		...
		if(ck_app){
			...
			if(CURRART === ART081){	//	사업통제	20150424_csh
				curAskType = ART082;
			}
			if(CURRART === ART091){	//	기획통제	20150424_csh
				curAskType = ART092;
			}
