package com.sds.acube.app.idir.org.user;

/**
 * UserTimeTableMap.java
 * 2002-11-06
 * 
 * 
 * 
 * @author kkang
 * @version 1.0.0.0
 *
 * Copyright 2001 Samsung SDS Co., Ltd. All rights reserved.
 */

import com.sds.acube.app.idir.org.common.*;

public class UserTimeTableMap extends TableMap
{
	// User Time Table Map
	public static final int USER_ID = 0;
	public static final int WHEN_CREATED = 1;
	public static final int WHEN_CHANGED = 2;
	public static final int WHEN_DELETED = 3;
	public static final int WHEN_DELETED_CONCURRENT = 4;
	public static final int WHEN_DELETED_PROXY = 5;
	public static final int WHEN_DELETED_DELEGATE = 6;
	public static final int LAST_LOGON = 7;
	public static final int LAST_LOGOUT = 8;
	public static final int LAST_LOGOUT_IP = 9;
	public static final int ASSIGNED_DATE = 10;

	public static final String m_strColumn[] =
	{
		"USER_ID",
		"WHEN_CREATED",
		"WHEN_CHANGED",
		"WHEN_DELETED",
		"WHEN_DELETED_CONCURRENT",
		"WHEN_DELETED_PROXY",
		"WHEN_DELETED_DELEGATE",
		"LAST_LOGON",
		"LAST_LOGOUT",
		"LAST_LOGOUT_IP",
		"ASSIGNED_DATE"
	};
	
	public static final int m_nDataType[] =
	{
		STRING,
		DATE,
		DATE,
		DATE,
		DATE,
		DATE,
		DATE,
		DATE,
		DATE,
		STRING,
		DATE
	};
		
	
	/**
	 * Get Column Name
	 * @param nColName   Column Type
	 * @return String 	  Column 명 
	 */	
	public static String getColumnName(int nColIndex)
	{
		return m_strColumn[nColIndex];
	}
	
	/**
	 * Get Column Data Type
	 * @param nColName   Column Type
	 * @return int 	  Column Data Type 
	 */	
	public static int getDataType(int nColIndex)
	{
		return m_nDataType[nColIndex];
	}	
	
	/**
	 * Data Type 반환
	 * @param strColumnName 	Table Column명 
	 * @return int
	 */
	public static int getDataType(String strColumnName)
	{
		String 	strUColumnName = "";
		int 	nDataType = -1;
		int 	nIndex = -1;
		
		strUColumnName = strColumnName.toUpperCase();
		
		for (int i = 0 ; i < m_strColumn.length ; i++)
		{
			if (m_strColumn[i].compareTo(strUColumnName) == 0)
			{
				nIndex = i;
			}		
		}
		
		if (nIndex >= 0)
		{
			nDataType = m_nDataType[nIndex];
		}
		
		return nDataType;
	}			
}
