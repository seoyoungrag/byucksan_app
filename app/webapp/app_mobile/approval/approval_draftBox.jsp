	...
	if(moreFlag != false){
		 pageSetting++;
	 	 $.ajax({
				...
				url:'<%=webUri%>/app/list/webService/approval/ListDraftBox.do',
...
function setDraftList(data){
	...
	if(pageSetting == 1){
		$("#approvalDraftList").html("");
	}
	
 	if(data.ListVos.length == 0){
		addHtml += "<li class='group'><a href=''>";
		addHtml += "<div class='rf'><p class='title'>기안문서 목록이 없습니다.</p></div></a>";		
	}else{
		...
		for(var i = 0; i < data.ListVos.length; i++){
			...
			addHtml += "<p class='status'><span><%=userName%></span><span>|</span><span>"+data.ListVos[i].draftdate+"</span></p>" ;
			addHtml +=	"</div></a><div class='signWrap'><button class='signBtn'>"+docStatus+"</button></div></li>";		
		} 
	}		 
 	
 	processCount += data.ListVos.length;
	$("#approvalDraftList").html(addHtml);
...
function viewAppDoc(docid){
	...
	document.location.href = '<%=webUri%>/app/list/webservice/approval/selectAppDoc.do?docId='+docid+'&D1='+d1+'&lobCode=LOB000';				
	...
	<div id="wrapper" style="overflow:hidden;">
		<div id="scroller">     
			...
			<div class="sub_content app">
				<ul class="list V_Limg_list Rdiv group" id="approvalDraftList">
		<!-- 			<li class="group">
						<a href="#a">
			...
			<div class="moreNtop">
				<a href="#none" class="more_bottom" onclick="getDraftList();">더보기 <span id="listCount"></span><span>/</span><span id="listTotalCount"></span></a>
