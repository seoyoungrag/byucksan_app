	...
	if(moreFlag != false){
	     pageSetting++;
	 	 $.ajax({
				...
				url:'<%=webUri%>/app/list/webService/approval/ListRearBox.do',
...
function setRearList(data){
	
	if(pageSetting == 1){
		$("#approvalRearList").html("");
	}
	
 	if(data.ListVos.length == 0){
		addHtml += "<li class='group'><a href=''>";
		addHtml += "<div class='rf'><p class='title'>후열문서 목록이 없습니다.</p></div></a>";		
	}else{
		...
		for(var i = 0; i < data.ListVos.length; i++){
			...
			addHtml +=	"</div></a><div class='signWrap'><button class='signBtn'></button></div></li>";		
		} 
	}
 	
 	processCount += data.ListVos.length;
	$("#approvalRearList").html(addHtml);	
...
function viewAppDoc(docid){
	...
	document.location.href = '<%=webUri%>/app/list/webservice/approval/selectAppDoc.do?docId='+docid+'&D1='+d1+'&lobCode=LOB013';				
	...
	<div id="wrapper" style="overflow:hidden;">
		<div id="scroller">        
			...
			<div class="sub_content app">
				<ul class="list V_Limg_list Rdiv group" id="approvalRearList">
					<!-- <li class="group">
						<a href="#a">
						...
						<div class="signWrap"><button onclick="#a" class="signBtn">결재완료</button></div>
					</li>
					<li class="group">
						<a href="#a">
		                	<img src="../img/proimg_sample.png" alt="이미지" class="circleImg"/>
						...
						<div class="signWrap"><button onclick="#a" class="holdBtn">보류</button></div>
					</li> -->
				</ul>
			</div>
		
			<div class="moreNtop">
				<a href="#none" class="more_bottom" onclick="getRearList();">더보기 <span id="listCount"></span><span>/</span><span id="listTotalCount"></span></a>
