	...
	if(moreFlag != false){
		 pageSetting++;
	 	 $.ajax({
				...
				url:'<%=webUri%>/app/list/webService/approval/ListApprovalCompleteBox.do',
...
function setCompleteList(data){
	...
	if(pageSetting == 1){
		$("#approvalCompleteList").html("");
	}
	
 	if(data.ListVos.length == 0){
		addHtml += "<li class='group'><a href=''>";
		addHtml += "<div class='rf'><p class='title'>완료함 목록이 없습니다.</p></div></a>";		
	}else{
	...
	$("#approvalCompleteList").html(addHtml);
...
function viewAppDoc(docid){
	...
	document.location.href = '<%=webUri%>/app/list/webservice/approval/selectAppDoc.do?docId='+docid+'&D1='+d1+'&lobCode=LOB010';				
	...
	<div id="wrapper" style="overflow:hidden;">
		<div id="scroller">   
			...
			<div class="sub_content app">
				<ul class="list V_Limg_list Rdiv group" id="approvalCompleteList">
					<!-- <li class="group">
						<a href="#a">
			...
			<div class="moreNtop">
				<a href="#none" class="more_bottom" onclick="getCompleteList();">더보기 <span id="listCount">20</span><span>/</span><span id="listTotalCount">55</span></a>
