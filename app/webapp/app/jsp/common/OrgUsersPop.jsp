...
function insertUser(results){
	...
	for(var i = 0; i < results.length; i++){
		var user = results[i];
		var row = "<tr style='cursor:hand;' id='"+user.userUID+"' deptID='"+ user.deptID+"' deptName='"+ user.deptName +"' userName='"+ user.userName;
		...
		row += "<td class='ltb_center' style='border-bottom: #ADBED7 1px solid;cursor:hand;' width='100'>"+userPosition+"</td>";
		row += "<td class='ltb_center' style='border-bottom: #ADBED7 1px solid;border-left:1pt solid #ADBED7;border-right:1pt solid #ADBED7;cursor:hand;'>"+user.userName+"</td></tr>";
		tbl.append(row);
	}		
}
...
function sendOk(){
	...
	if(trUser !== null){
		...
		if(confirmYn === "Y"){
			if(!confirm("<spring:message code='common.msg.adduser' />")){
				return;
			}
		}
...
<%if(!"Y".equals(apprParam)){ %>
	if (opener != null && opener.setUserInfo) {
		var msg = opener.setUserInfo(user);
<% }else{ %>
	if (opener != null && opener.setUserOfficeApprInfo) {
		var msg = opener.setUserOfficeApprInfo(user);
<% } %>
...
<div class="pop_title02">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="pop_table05">
		<tr>
			<td>
			<span class="pop_title77"><spring:message code="common.title.userlist" /></span></td>
		<!-- 여백 시작 -->
