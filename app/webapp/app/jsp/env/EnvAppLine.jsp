...
<%
/** 
 *  Class Name  : EnvAppLine.jsp 
 *  Description : 결재경로그룹 관리 - 결재경로(생산)
...
<style type="text/css">
	...
	.range {display:inline-block;width:85px;height:25px; !important;}
</style>
...
function getApproverList(lineinfos) {
	...
	for (var loop = 0; loop < linelength; loop++) {
		if (lineinfo[loop].indexOf(String.fromCharCode(2)) != -1) {
			...
			approver.changeYn = info[27];
			approver.lineNum = info[28];
...
$(document).ready(function() { 
	...
	if (opener != null && opener.getGroupType() != "GUT004") {
		$('#spanNotice').text("* <spring:message code='env.option.txt.08'/>");
	}
...
function insertUser(results){
	...
	if(results.length > 0){
		for(var i = 0; i < results.length; i++){
			...
			row += "' roleCodes='"+ user.roleCodes + "' rowOrd='"+ i;
			...
			row += "<td class='ltb_center' width='107'>"+userPosition+"</td>";
			row += "<td class='ltb_center'>"+user.userName+"</td></tr>";
			tbl.append(row);
		}
	} else{
		var row = "<tr bgcolor='#ffffff'>";
		row += "<td colspan='2' class='ltb_center' style='border-bottom: "+tbColor+" 1px solid;'><spring:message code='app.alert.msg.19' /></td>";
		row += "</tr>";
		tbl.append(row);
	}		
}
...
function setAppLine(){
	...
	if (opener != null && opener.getAppLine && opener.getMode) {
		if (opener.getMode() == "I") {
			g_lines = opener.getInitTxtAppLines();
		...
		for(var i = 0; i < arrLines.length; i++){
			...
			var id = "", lineNum = "" , lineOrder = "" , lineSubOrder = "" ,  approverId = "" , approverName  = ""
				...
				, lineHisId = "", fileHisId = "", bodyHisId = "", approverRole = "", positionName  = "", deptName = "", userName = "", opt_nm = "", changeYn = "" ;
			...
			changeYn				= line.changeYn;

			//부서협조,부서감사,부서합의
			//현재 결재 경로가 부서협조,부서합의, 부서감사 결제 인지 확인
	...
	for(var i = 0; i < appColl.length; i++){
		...
		if(ck_app){
			...
			if(appLines.length === 0){//처음입력이면(즉 신규 기안서이면)
				...
				if(radio.length == 0){
					radio = $('input#'+ART020+', input#'+ART034+', input#'+ART043);
					if(radio.length > 0){
						radio.attr('checked',true);
						opt_code = radio.val();
...
function approveMakeRow(tbId, id, lineNum , lineOrder , lineSubOrder ,  approverId , approverName 
	...
	var row = "<tr ";
	row += "id='"+ tbId + "_" + id + "' lineNum='1' lineOrder='" + lineOrder + "' lineSubOrder='" + lineSubOrder + "' "; 
	...
	row += "lineHisId='" + lineHisId +"' fileHisId='"+ fileHisId +"' bodyHisId='"+bodyHisId +"'  approverRole='" + approverRole + "' "; // jth8172 2012 신결재 TF
	row += " onclick='onLineClick($(\"#"+ tbId + "_" + id+"\"));' onmousemove='onLineMouseMove();' onkeydown='onLineKeyDown($(\"#"+ tbId + "_" + id +"\"));'";
	row += " style='background-color:"+tbgcolor+";' >";
	row += "<td width='207' class='ltb_center' style='border-bottom: #E3E3E3 1px solid; '>" + (deptName === ""?"&nbsp;":deptName) + "</td>";
	row += "<td width='130' class='ltb_center' style='border-bottom: #E3E3E3 1px solid;border-left:1pt solid #E3E3E3;border-right:1pt solid #E3E3E3; '>" + (positionName === ""? "&nbsp":positionName) + "</td>";
	row += "<td width='158' class='ltb_center' style='border-bottom: #E3E3E3 1px solid;border-right:1pt solid #E3E3E3; '>" + (userName === ""? "&nbsp":userName)  + "</td>"
	row += "<td class='ltb_center' style='border-bottom: #E3E3E3 1px solid;border-right:1pt solid #E3E3E3; '>" + (opt_nm === ""? "&nbsp":opt_nm) + "</td>";
	row += "</tr>";
	return row;
}
...
<%}else{%>
	...
	if((askType === ART010 || askType === ART053) && lineNum == "1"){ //기안 및 1인 결재
		rdType1.attr('disabled', false); //enable
		rdType2.attr('disabled', true);  //disable		
		chkType5.attr('disabled', false); //enable// jth8172 2012 신결재 TF
	}else if((askType === ART010 || askType === ART053) && lineNum == "2"){ //부서추가이면 
		rdType1.attr('disabled', false); //enable
		rdType2.attr('disabled', false);
...
function onAddDept(){
	...
	if(radios2.length > 0 && rdLineNum.length < 1){
		var askRadio = null;
		//부서 협조 및 부서 합의, 감사협조 둘중하나를 자동선택하게 한다.
		if(radios1.length === 0){
			askRadio = radios2[0];
		...
		//처리 부서인지 체크한다.**************************************************************************
		if(sdept.attr("isProcess") !== '1'){
		//	alert("결재선으로 지정될 수 없습니다.");
		//	return;
		}
...
function getApprovalLines(lines){
	...
	for( var i = max; i>=0 ; i--){
		...
		strAppline += line.getAttribute('approverDeptName') + String.fromCharCode(2) + line.getAttribute('askType')			+ String.fromCharCode(4);
	}
	return strAppline;	
}
...
function checkLineRole(){
	...
	for(var i = (lineCnt-1); i >= 0; i--){
		...
		if(oldItem !== null){
			...
			if(ARTSING === ART050 //결재 이후에는 협조, 부서협조, 후열만  올수 있다.
			...
			if(ARTSING === ART051 //전결 이후에는 협조, 부서협조, 후열만  올수 있다.
					<c:if test='${options["OPT336"].useYn == "Y"}'>
						&& tmpItem.getAttribute('askType') !== ART030 //협조
					</c:if>
					<c:if test='${options["OPT337"].useYn == "Y"}'>
			...
			if(ARTSING === ART052 //대결 이후에는 협조, 부서협조, 후열만  올수 있다.
					<c:if test='${options["OPT336"].useYn == "Y"}'>
						&& tmpItem.getAttribute('askType') !== ART030 //협조
					</c:if>
					<c:if test='${options["OPT337"].useYn == "Y"}'>
			...
			if(oldItem.getAttribute('askType') === ART054 && tmpItem.getAttribute('askType') !== ART054
					&& oldItem.getAttribute('lineNum') === tmpItem.getAttribute('lineNum') && tmpItem.getAttribute('lineNum') === "1"){ 
				//후열뒤에는 후열만 올 수 있음
			...
			if(oldItem.getAttribute('lineNum') !== tmpItem.getAttribute('lineNum') && tmpItem.getAttribute('lineNum') === "1"){
			...
			if( CURRART === ART032 || CURRART === ART033 || CURRART === ART034 || CURRART === ART035 || 
				...
				if(Art033Idx > ART050Idx){ //결제 이전에 존재
					if(Art035Idx < ART050Idx){
						alert('<spring:message code="approval.msg.applines.after.noorder" />');
						return;
					}
				}
				if(Art133Idx > ART050Idx){ //결제 이전에 존재// jth8172 2012 신결재 TF
				...
				if(tmpItem.getAttribute('askType') === ART034){
				...
				if(tmpItem.getAttribute('askType') === ART134){ // jth8172 2012 신결재 TF
			...
			if(CURRART === ART041 || CURRART === ART042 || CURRART === ART043 || CURRART === ART044){					
				...
				if(tmpItem.getAttribute('askType') === ART044){
					Art044Idx = i;
				}

				
				
				if(tmpItem.getAttribute('askType') === ART043){
			...
			if(oldItem.getAttribute('askType') === ART050 //결재이후  결재선 지정할 수 없음
				...
				alert('<spring:message code="approval.msg.applines.after2.art050" />');
				return false;
			}
				
			if(oldItem.getAttribute('askType') === ART051 //전결이후  결재선 지정할 수 없음
					&& oldItem.getAttribute('lineNum') === "2"){
				alert('<spring:message code="approval.msg.applines.after2.art051" />');
				return false;
			}

			if(oldItem.getAttribute('askType') === ART052 //대결이후 결재선 지정할 수 없음
					&& oldItem.getAttribute('lineNum') === "2"){
				alert('<spring:message code="approval.msg.applines.after2.art052" />');
				return false;
			}
			
			if(oldItem.getAttribute('askType') === ART053 //1인전결 이후  결재선 지정할 수 없음
					&& oldItem.getAttribute('lineNum') === "2"){
				alert('<spring:message code="approval.msg.applines.after2.art053" />');
				return false;
			}
		}else{
		...
		if((tmpItem.getAttribute('askType') === ART050 ||tmpItem.getAttribute('askType') === ART051 
			...
			appItem = tmpItem;
			if(tmpItem.getAttribute("approverRole") === role_ceo){
	...
	//부서협조 중복 체크
	if(CURRART === ART032 || CURRART === ART033 || CURRART === ART034 || CURRART === ART035){
		var ART033Cnt = $('#tbApprovalLines tbody tr[askType="'+ART033+'"]').length;
		var ART035Cnt = $('#tbApprovalLines tbody tr[askType="'+ART035+'"]').length;
	
		if(ART033Cnt > 1 || ART035Cnt > 1){ //1개 이상이면 결제 순서가 잘못된 것임
	...
	//부서합의 중복 체크  // jth8172 2012 신결재 TF
	if(CURRART === ART132 || CURRART === ART133 || CURRART === ART134 || CURRART === ART135){ 
		var ART133Cnt = $('#tbApprovalLines tbody tr[askType="'+ART133+'"]').length;
		var ART135Cnt = $('#tbApprovalLines tbody tr[askType="'+ART135+'"]').length;
	
		if(ART133Cnt > 1 || ART135Cnt > 1){ //1개 이상이면 결제 순서가 잘못된 것임
			alert('<spring:message code="approval.msg.applines.after.errappline" />');
			return;
		}
	...
	if(CURRART === ART041 || CURRART === ART042 || CURRART === ART043 || CURRART === ART044){
		...
		if(ART042Cnt > 1 || ART044Cnt > 1){ //1개 이상이면 결제 순서가 잘못된 것임
			alert('<spring:message code="approval.msg.applines.after.errappline" />');
			return;
		}
...
<acube:outerFrame>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="pop_table05">
		<tr>
			<td>
			<span class="pop_title77"><spring:message code="approval.title.applines" /></span></td></tr>
	<!-- 여백 시작 -->
...
<div>
	...
	<tr>
    	<td colspan="2" height="3" background="<%=imagePath%>/dot_search1.gif"></td>
    </tr>
	<tr>		
		<td width="50%">		
	...
	<tr>
		...
		<tr>
			<td height="185">
			<div id="org_tree" style="height:185px; overflow:auto; background-color : #FFFFFF;">
		    <ul>
...
<!--				</li> -->
		...
		<tr>
			...
			<div style="height:154px; overflow-x:hidden; overflow-y:scroll; background-color : #FFFFFF;">
			<table id="tbUsers" width="100%" bgcolor="#e3e3e3" border="0" cellpadding="0" cellspacing="0">
				<tbody />
			</table>
			</div>
		</td>
 ...
 <!-- 결재 옵션 -->
		...
		<tr>
			<td width="100%" height="100%" class="g_box" style="padding:10px 10px 10px 10px;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						<td>
							<div id='divART032' height="30" style="display:none; height:30px; "><!-- ==============부서 협조============================== -->
							...
							<div id='divART132' style="display:none; height:30px; "><!-- ==============부서 합의============================== -->
								...
								<span class="range">
								<input id='ART133' name="options1" type="radio"  value='<%=appCode.getProperty("ART133", "ART133", "APP") %>' codeNm='${options["OPT056"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT056"].optionValue}</nobr>
								</span>
								</c:if><!-- 합의(기안) // jth8172 2012 신결재 TF-->
								<c:if test='${options["OPT057"].useYn == "Y"}'> 
								<span class="range">
								<input id='ART134' name="options1" type="radio"  value='<%=appCode.getProperty("ART134", "ART134", "APP") %>' codeNm='${options["OPT057"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT057"].optionValue}</nobr>
								</span>
								</c:if><!-- 합의(검토) // jth8172 2012 신결재 TF-->
								<c:if test='${options["OPT058"].useYn == "Y"}'> 
								<span class="range">
								<input id='ART135' name="options1" type="radio"  value='<%=appCode.getProperty("ART135", "ART135", "APP") %>' codeNm='${options["OPT058"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT058"].optionValue}
								</span>
							...
							<div id='divART041' height="30" style="display:none; height:30px; "><!-- ==============부서 감사============================== -->
								...
								<input id='ART042' name="options1" type="radio"  value='<%=appCode.getProperty("ART042", "ART042", "APP") %>' codeNm='${options["OPT011"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT011"].optionValue}</nobr>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								</c:if><!-- 감사(기안) -->
							...
							<!-- 기안 , 1인 전결 , 검토, 협조, 병렬협조, 부서협조 -->
							<div id='divART010' style="height:50px;">								
								...
								<span class="range">
								<nobr><input id='ART130' name="options1" type="radio"  value='<%=appCode.getProperty("ART130", "ART130", "APP") %>' codeNm='${options["OPT053"].optionValue}' onclick="optClick(this)" />${options["OPT053"].optionValue}</nobr>
								</span>
								</c:if><!-- 개인순차함의// jth8172 2012 신결재 TF-->
								<c:if test='${options["OPT054"].useYn == "Y"}'>
								<span class="range">
								<input id='ART131' name="options1" type="radio"  value='<%=appCode.getProperty("ART131", "ART131", "APP") %>' codeNm='${options["OPT054"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT054"].optionValue}</nobr>
								</span>
								</c:if><!-- 개인병렬합의// jth8172 2012 신결재 TF-->
								<c:if test='${options["OPT055"].useYn == "Y"}'>
								<span class="range" >
								<input id='ART132' name="options1" type="radio"  value='<%=appCode.getProperty("ART132", "ART132", "APP") %>' codeNm='${options["OPT055"].optionValue}' onclick="optClick(this)" /><nobr>${options["OPT055"].optionValue}</nobr>
								</span>
								</c:if><!-- 부서합의// jth8172 2012 신결재 TF-->
							</div>
							<!-- 결재, 전결, 대결, 감사, 부서감사, 후열  -->
							<div id='divART050' style="height:25px;">
								
								<c:if test='${options["OPT014"].useYn == "Y"}'> 
								<span class="range" style="width:45px" ><!-- 결재 -->
								...
								<span class="range" style="width:45px" ><!-- 전결 -->
								...
								<span class="range" style="width:45px" ><!-- 대결-->
	...
	<table border='0' cellpadding='0' cellspacing='0' width="100%">
		<tr>
			<td width="60%">&nbsp;<span id="spanNotice"></span></td>
			<td width="40%" height="23" align="right" valign="bottom">			
	...
	<table  width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td>
				<acube:tableFrame class="table_border">
					<tr>
						<td width="100%" height="100%">
							<div>
							<acube:tableFrame class="table">
									<tr>
										<td width="207" class="ltb_head">
											<spring:message code="approval.table.title.sosok" />
										</td>
										<td width="130" class="ltb_head">
											<spring:message code="approval.table.title.gikwei" />
										</td>
										<td width="158" class="ltb_head">
											<spring:message code="approval.table.title.name" />
										</td>
										<td class="ltb_head">
							...
							<div style="height:120px; overflow-x:hidden; overflow-y:scroll; background-color:#FFFFFF;">
							<table bgcolor="#bbbbbb" id="tbApprovalLines" width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed;">
								<tbody />
							</table>
							</div>
						</td>
