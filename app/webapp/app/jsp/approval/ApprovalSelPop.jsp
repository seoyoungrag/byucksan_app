...
<%
/** 
 *  Class Name  : ApprovalPreReaderPop.jsp
...
<%
	...
	String reeceiverOrder = (String) request.getParameter("reeceiverOrder");
...
var tbColor = "<%=tbColor%>";
var docId = "<%=docId%>";
var receiverOrder = "<%=reeceiverOrder%>";
...
function onListDblClick(obj){
	document.selection.empty();
	// 2015.07.28_lsk_더블 클릭시 결재자 지정 주석 처리
	// addApprovalLine();
}
...
function sendOk(){
	...
	$.post("<%=webUri%>/app/enforce/listProcessEnfDoc.do", "docId="+docId+"&enfLines="+appline+"&receiverOrder="+receiverOrder, function(data){
		
		if (data.result > 0 ) {
			$.post("<%=webUri%>/app/enforce/enfline/insertListEnfLine.do", "docId="+data.count+"&enfLines="+appline, function(data){
		    	
		        if (data.result =='OK') {
					
		        	alert("<spring:message code='approval.result.msg.approverfixok'/>");
		        	if ( opener.getLineOpenType() != 'C' ) {
		        		opener.listRefresh();
		        	}
		        	exitAppDoc();
		        }
	...
	window.close();
	if ( opener != null && opener.exitAppDoc != null ) {
		opener.exitAppDoc();	
	}
...
function checkLineRole(){
	...
	for(var i = 0; i < max; i++){
		...
		if(i > 0){
//			if(nAskType > tmpAsk){		//	선람->담당에서 담당->선람으로 순서변경		20150330_csh
			if(nAskType < tmpAsk){
