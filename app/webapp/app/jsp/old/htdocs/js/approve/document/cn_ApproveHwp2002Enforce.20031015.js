...
//	g_bIsEnforceModified = (g_strEditType == "0");
	...
	else
	{
		...
		if (strFileName != "")
		{
			...
			{
				Enforce_HwpCtrl.Open(strLocalPath, "", "lock:FALSE");
			}
			else
			{
...
function makeEnforceDocument(strEnforceFormUrl, nCaseNumber)
{
	...
	if (objEnforceFile == null)
	{
		if (strEnforceFormUrl != "")
		{
			...
			{
				Enforce_HwpCtrl.Open(g_strDownloadPath + strEnforceFormUrl, "", "lock:FALSE");
// 20030801
				setEnforceXmlDataToForm();
...
//			setXmlDataToForm();
		...
		{
			if (Enforce_HwpCtrl.FieldExist(arrSrcField[i]) && !isAutoSetField(arrSrcField[i]))
			{
...
function setEnforceRecipientsToForm()
{
	...
	if (objRecipGroup != null)
	{
		...
		if (Enforce_HwpCtrl.FieldExist(strDeptField))
		{
		...
		if (Enforce_HwpCtrl.FieldExist(strRefField))
		{
...
*/
								...
								if (Enforce_HwpCtrl.FieldExist(strFieldName) || Enforce_HwpCtrl.FieldExist(strUnionField) ||
									Enforce_HwpCtrl.FieldExist(FIELD_STAMP) || Enforce_HwpCtrl.FieldExist(FIELD_SENDER_AS + "#1"))
								{
									...
									else if (Enforce_HwpCtrl.FieldExist(strUnionField))
										Enforce_HwpCtrl.RenameField(strUnionField, strFieldName);
...
*/
								...
								strFieldName == FIELD_ENFORCE_OMIT_STAMP)
						{
							...
							{
								if (Enforce_HwpCtrl.FieldExist(strUnionField))
									Enforce_HwpCtrl.RenameField(strUnionField, strFieldName);
						...
						else
						{
									...
									strFieldName == FIELD_ENFORCE_OMIT_STAMP)
							{
								...
								if (strFieldName == FIELD_DEPT_OMIT_STAMP)
									...
									strUnionField = FIELD_DEPT_OMIT_STAMP;
								strUnionField = (strFieldName == FIELD_DEPT_OMIT_STAMP ? FIELD_COMPANY_OMIT_STAMP : FIELD_DEPT_OMIT_STAMP);
								if (Enforce_HwpCtrl.FieldExist(strUnionField) || Enforce_HwpCtrl.FieldExist(FIELD_OMIT_STAMP))
								{
									if (Enforce_HwpCtrl.FieldExist(strUnionField))
										Enforce_HwpCtrl.RenameField(strUnionField, strFieldName);
									else if (Enforce_HwpCtrl.FieldExist(FIELD_OMIT_STAMP))
				...
				if (getApproverSerialOrder(objApprover) >= getApproverSerialOrder(getCompleteApproverInApprovalLine(objApprovalLine)))
				{
					if (getApproverIsSigned(objApprover) == "Y")
					{
						...
						if (strSignDate != "")
						{
							...
							strFieldList += strApprovalField + "\u0002";
							strValueList += Enforce_HwpCtrl.GetFieldText(strApprovalField) + " " + strDate + "\u0002";
						}
					}
				}
			}
