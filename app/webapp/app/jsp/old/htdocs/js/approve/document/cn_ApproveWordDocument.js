...
{
	alert('cn_ApproveWordDocument.js 758  16. 1. 21 오후 5:12  jyd');
	loadDocument();
}
...
{
	var lVersion = parseFloat(g_DocWord.Application.Version);
	// Office 2K(Ver 9.x) 이하인 경우에만 저장, Office XP(Ver 10.x)에서는 오류발생
	if (lVersion < 10.0)
		g_DocWord.Save();
...
//				g_strSenderAs = readTempRecordText(FIELD_SENDER_AS + "_P1");
...
function changeFormUrl(strFormUrl)
{
	...
	openDocument(g_strDownloadPath + strFormUrl);
/*
...
function getFieldText(strFieldName)
{
	...
	var nStartPos = g_DocWord.Bookmarks(strFieldName).Range.Cells(1).Range.Start;
	var nEndPos = g_DocWord.Bookmarks(strFieldName).Range.Cells(1).Range.End;
	var strFieldText = g_DocWord.Range(nStartPos, nEndPos-1).Text;
	if (g_strEditType != 0)
		g_DocWord.Protect(1);
				...
				if (getApproverSerialOrder(objApprover) >= getApproverSerialOrder(getCompleteApproverInApprovalLine(objApprovalLine)))
				{
					if (getApproverIsSigned(objApprover) == "Y")
					{
						...
						if (strSignDate != "")
						{
							...
							if ((readTempRecordText(strApprovalField)).indexOf(strMonthNDay) == -1)
								strValueList += readTempRecordText(strApprovalField) + " " + strMonthNDay + "\u0002";
							else
								strValueList += readTempRecordText(strApprovalField) + "\u0002";
...
// 2003.04.01 for 연합캐피탈
		if (g_bIsFormData == true)
		{
//			if (strField.indexOf("P_") != -1 || strField.indexOf("D_") != -1 ||
//				strField.indexOf("N_") != -1 || strField.indexOf("기안일자1") != -1)
			{
				if (g_DocWord.Bookmarks.Exists(strField))
				{
					var myRange = g_DocWord.Bookmarks(strField).Range.Cells(1).Range;
...
//	parent.HttpTransfer1.SaveToLocalFile(strFilePath, strHtmlFieldString);
	parent.saveToLocalFile(strFilePath, strHtmlFieldString);
...
function insertOpinionToBody(strOldOpinion, strOpinion)
{
	...
	else
	{
		if (g_strLineName == "0")
		{
			if (g_strLineName == "0")
			{
				if (g_DocWord.Bookmarks.Exists(LABEL_EXIST_OPINION))
				{
...
{
	if (g_DocWord.Bookmarks.Exists(LABEL_EXIST_OPINION))
		putFieldText(LABEL_EXIST_OPINION,"");
}
...
//	strProposalString = strHtmlString + strProposalString;
	...
	parent.saveToLocalFile(strFilePath, strProposalString);
/*
	var strFileName = getAttachFileName(getBodyFileObj());
	var strLocalPath = g_strDownloadPath + strFileName;
	var strServerUrl = g_strUploadUrl + strFileName;
...
//	g_DocWord.SaveAs(g_strDownloadPath + strTitle);
	var strDownloadPath = g_strDownloadPath.substring(0, g_strDownloadPath.length-1);
	strDownloadPath = strDownloadPath.substring(0, strDownloadPath.lastIndexOf("\\"));
	strDownloadPath = strDownloadPath.substring(0, strDownloadPath.lastIndexOf("\\")+1);
	while(strTitle.indexOf("/") != -1)
	{
		strTitle = strTitle.replace("/","-");
	}
	var strFilePath = saveFileDialog(strDownloadPath + strTitle, "");
	if (strFilePath.length == 0)
		return false;
//	g_DocWord.Activate();
//	g_AppWord.Dialogs(84).Show();
	g_DocWord.SaveAs(strFilePath);

	if (getBodyFileObj() != null)
		g_DocWord.SaveAs(g_strDownloadPath + getAttachFileName(getBodyFileObj()));
}
...
function setFormDataToXml()
{
	...
	while(nIndex <= nBodyCount)
	{
		...
		if (g_DocWord.Bookmarks.Exists(FIELD_SENDER_AS + "_P" + nIndex) == true)
		{
			var strSenderAs = readTempRecordText(FIELD_SENDER_AS + "_P" + nIndex);
...
function printDocument()
{
	...
	// 의견정보를 입력한다.
//	g_DocWord.Content.InsertAfter(GetOpinionForPrint());
...
{
	var strDocName = g_DocWord.Name;
	var strTempUrl = parent.getDownloadPath() + parent.getGUID() + ".doc";

	if (confirm(CONFIRM_PRINT_DOCUMENT) == false)
		return;

	g_DocWord.SaveAs(strTempUrl);
	saveDocument();
	...
	g_AppWord.Dialogs(97).Show();

	// Background true, false.
	if (objWordApp.Documents(1).PrintOut(false) == false)
		return false;

	// WdSaveOptions constants : wdDoNotSaveChanges, wdPromptToSaveChanges, wdSaveChanges.
	if (objWordApp.Documents(1).Close(0) == false)
		return false;
...
function GetOpinionForPrint()
{
	var bExistOpinion = false;
	var strOpinionList = "";
	var objApprovalLine = null;
	...
	while (objApprover != null)
	{
		...
		if (strOpinion != "")
		{
			...
			if (bExistOpinion)
				strOpinionList += strApproverUserName + " : " + strOpinion + "\r\n";
			else
				strOpinionList += "\r\n\r\n" + strApproverUserName + " : " + strOpinion + "\r\n";
...
function onSetViewScale(bExpand)
{
	...
	var objApprDoc = document.getElementById("wordObject");
	var objEnfDoc = document.getElementById("enforceObject");
	if (objApprDoc != null)
		objApprDoc.Document.ActiveWindow.View.Zoom.Percentage = nFactor;
	if (objEnfDoc != null)
		objEnfDoc.Document.ActiveWindow.View.Zoom.Percentage = nFactor;
}

function ReplaceSpecialChar(strFileName)
{
