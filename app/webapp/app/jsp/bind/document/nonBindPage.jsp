...
<script LANGUAGE="JavaScript">
	...
	function onSubmit() {
		...
		form.action = '<%=webUri%>/app/bind/document/nonBind.do';
		form.submit();
	}
	
	function changePage(pageNo) {
		document.listForm.action = '<%=webUri%>/app/bind/document/nonBind.do';
...
<acube:outerFrame>
	<form name="listForm" method='post' action='<%=webUri%>/app/bind/document/nonBind.do'>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td colspan="2">
				<acube:titleBar>
				<c:choose>
					<c:when test="${empty bindName}"><spring:message code='bind.title.non.box' /></c:when>
