...
<% if(roleCode.indexOf(systemAdminCode) > -1 || roleCode.indexOf(appSystemAdminCode) > -1) { %>	
	...
	function goSelectComp() {
		...
		form.action = '<%=webUri%>/app/bind/unit/ex/listDeptBind.do?deptId=' + '${compId}' + '&adminSelectDept=N';
		form.target = '_self';
		form.submit();
	}
	...
	function setDeptInfo(obj) {
		if (typeof(obj) == "object") {
			if('ChangeDept' == openType) {
				...
				form.searchType.value = "";
				form.searchValue.value = "";
				form.action = '<%=webUri%>/app/bind/unit/ex/listDeptBind.do?deptId=' + obj.orgId + '&adminSelectDept=Y';
				form.target = '_self';
				form.submit();
			} 
		}
...
<acube:outerFrame>
	<form id="listForm" name="listForm" method='post' action='<%=webUri%>/app/bind/unit/ex/listDeptBind.do'>
						...
						<tr>
							...
							<%
							    try {
									    ...
									    acubeList = new AcubeList(sLine, 3);  // 왼쪽리스트 함 컬럼 수정  20150603_jskim
									    acubeList.setColumnWidth("20,400,*");
									    ...
									    titleRow.setData(++rowIndex, m.getMessage("bind.obj.brunit", null, langType));
									    ...
									    String style = "text-overflow : ellipsis; overflow : hidden;  height: 30px;";    //  row 높이 변경 20150528_jskim 
									    ...
									    if (rows.size() == 0) {
											...
											for (int i = 0; i < rows.size(); i++) {
											    ...
											    row.setData(rowIndex, "<input type=\"checkbox\" name=\"bindChk\" value=\"" + vo.getUnitId()   // 왼쪽리스트 함 컬럼 수정  20150603_jskim
											    		+ "\" compId=\"" + vo.getCompId() 
											    		+ "\" deptId=\"" + vo.getDeptId() 
											    		+ "\" unitId=\"" + vo.getUnitId() 
											    ...
											    row.setData(++rowIndex, vo.getUnitId());   
											    ...
											    row.setAttribute(rowIndex, "style", "text-align:center; line-height:28 px;");
