...
<script LANGUAGE="JavaScript"><!--
	...
	function goAddPage() 
	{
		...
		openWindow('bind_unit_add_win', '<%=webUri%>/app/bind/unit/simple/add.do?'+params , 480, 350, 'no');
	}
	...
	function goEditPage() 
	{
		...
		if(items.length == 0) {
			...
			openWindow('bind_unit_edit_win', '<%=webUri%>/app/bind/unit/simple/edit.do?'+params , 480, 350, 'no');
		}
	}
	...
	function goInfoPage(_unitId) 
	{
		...
		if(items.length == 0) {
			...
			openWindow('bind_unit_info_win', '<%=webUri%>/app/bind/unit/simple/info.do?'+params , 480, 360, 'no');
		}
	}
...
<% if(roleCode.indexOf(systemAdminCode) > -1 || roleCode.indexOf(appSystemAdminCode) > -1) { %>	
	...
	function goSelectComp() {
		...
		form.action = '<%=webUri%>/app/bind/unit/simple/list.do?deptId=' + '${compId}' + '&adminSelectDept=N';
		form.target = '_self';
		form.submit();
	}
	...
	function setDeptInfo(obj) {
		if (typeof(obj) == "object") {
			if('ChangeDept' == openType) {
				...
				form.action = '<%=webUri%>/app/bind/unit/simple/list.do?deptId=' + obj.orgId + '&adminSelectDept=Y';
				form.target = '_self';
				form.submit();
			} 
		}
...
<acube:outerFrame>
	<form name="listForm" method='post' action='<%=webUri%>/app/bind/unit/simple/list.do'>
	<input type="hidden" id="adminSelectDept" name="adminSelectDept" value=${adminSelectDept} />
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td colspan="2" nowrap="nowrap">
				<acube:titleBar><spring:message code="bind.title.unit.list" /></acube:titleBar>
			</td>
		</tr>
		<tr>
							    ...
							    try {
									    ...
									    if (rows.size() == 0) {
											...
											for (int i = 0; i < rows.size(); i++) {
											    ...
											    row.setData(++rowIndex, BindUtil.getDateFormat(vo.getApplied(), dateFormat));
											    row.setAttribute(rowIndex, "style", style);
