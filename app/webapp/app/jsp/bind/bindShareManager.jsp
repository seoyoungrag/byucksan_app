...
<script LANGUAGE="JavaScript">
	...
	function onSubmit() {
		var form = document.listForm;
		/* form.pageNo.value = 1; */
		form.action = '<%=webUri%>/app/bind/bindShareManager.do';
		form.target = '_self';
		form.submit();
	}
...
<% } %>	
	...
	function fncBindTreeRefresh() {
		...
		params = 'deptId=' + deptId + '&bindType=ALL' + '&searchYear=2016';
		
		var url = '<%=webUri%>/app/bind/bindListShare.do?'+params;
	...
	fncEditReload = function(bindId) {
	    ...
	    var url = '<%=webUri%>/app/bind/editShare.do?'+params;
	...
	fncAddReload = function() {
	    ...
	    var url = '<%=webUri%>/app/bind/addShare.do?'+params;
	...
	function goObsoletePage() {
		...
		if(confirm(confirmMessage)) {
			var form = document.listForm;
			var url = '<%=webUri%>/app/bind/bindShareObsolete.do';
