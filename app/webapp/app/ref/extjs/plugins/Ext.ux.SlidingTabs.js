...
Ext.ux.DDSlidingTabs = Ext.extend( Ext.dd.DDProxy, {
    ...
    reorderTab: function() {
		...
		Ext.Ajax.request({
			url: 'user.do',
			params: { 
				forwardName: 'setTabOrder',
				'dms:tabsOrder': tabsId.toString()
			},

			success: function(frm, action) {
				var data = Ext.util.JSON.decode(frm.responseText);
				if(data.success) {
				} else {
					App.util.showBasicError(m.get('dms:default_error_msg') + '<br>' + data.msg);
				}
			},

			failure: function(frm, action) {
				App.util.showBasicError(m.get("dms:default_error_msg"));
			}
		});
